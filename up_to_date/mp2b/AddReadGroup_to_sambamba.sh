...
RGCN=`samtools view -H $OLD_BAM | grep '@RG' | gawk 'NR==1{ if (match($0,/CN:[ A-Za-z0-9_-]*/,m)) print m[0] }' | sed 's/CN://' | sed 's/ /_/g'`
...
JOB1="java -jar /cvmfs/soft.computecanada.ca/easybuild/software/2017/Core/picard/2.20.6/picard.jar AddOrReplaceReadGroups VALIDATION_STRINGENCY=LENIENT \
INPUT=$BAM \
