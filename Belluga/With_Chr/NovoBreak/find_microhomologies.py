...
def moduleload():
        ...
        subprocess.run(["bash", "moduleload.sh"])
...
def identifymicrohomologies(consensus_length,blastfile_name):
        ...
        for line in blast_results_df.itertuples(index=True, name='Pandas'):
                ...
                if i == 2:
                        break
        microhomology_length = blast_length - consensus_length
...
def makefastafile(reference,position1,position2):
        SEQUENCE1 = subprocess.check_output(["bash", "getSequence.sh",reference,position1])
        SEQUENCE2 = subprocess.check_output(["bash", "getSequence.sh",reference,position2])
        #SEQUENCE1 = subprocess.run(["bash","getSequence.sh",reference,position1])
        #SEQUENCE2 = subprocess.run(["bash","getSequence.sh",reference,position2])

        #print (SEQUENCE1.decode('utf-8'))
        #print (SEQUENCE2.decode('utf-8'))
...
def makeblastgenomedb(reference,database):


        subprocess.run(["bash","makeblastgenomedb.sh",reference, database])
        return

def makeblastcustomdb(fasta_file,ID):

        customdb_name = ID + ".blast.tmp.db"
        subprocess.run(["bash","makeblastcustomdb.sh",fasta_file,customdb_name])
        return customdb_name

def blastn(query,database):

        blastfile_name = query + "_blast.out"
        subprocess.run(["bash","blastn.sh",query,database])
...
#check if database is there already (if dir is already created)
        ...
        if os.path.exists(database_directory)==False:
                ...
                makeblastgenomedb(reference,database)
                print ("Done")
...
##reading VCF file as panda dataframe
        ...
        ## for now I extend each regions 100bp on each side, it could be better to check orientation and extend on one side only
                ...
                POS = getattr(row, "CHR1")+":"+str(POS1)+"-"+str(POS2)

                END1 = getattr(row,"END")-100
                END2 = getattr(row,"END")+100
                END = getattr(row, "CHR2")+":"+str(END1)+"-"+str(END2)

                print ("Making fasta file")
                makefastafile(reference,POS,END)
                print ("Done")

                print ("Making custom database")
                customdbname = makeblastcustomdb('regions.tmp.fa',ID)
                print ("Done")
                ...
                blastfile_name = blastn(query_file,customdbname)
                print ("Done")
