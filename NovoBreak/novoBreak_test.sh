...
# sbatch --job-name=novoBreak_${NOPATHNAME} --output=%x-%j.out --time=24:00:00 --mem=256G \
...
echo "#!/bin/sh" > $JOB_OUTPUT_DIR/${STEP}/${NOPATHNAME}.sh
echo "$COMMAND" >> $JOB_OUTPUT_DIR/${STEP}/${NOPATHNAME}.sh
sbatch --job-name=novoBreak_${NOPATHNAME} --output=%x-%j.out --time=24:00:00 --cpus-per-task=45 --mem=256G $JOB_OUTPUT_DIR/${STEP}/${NOPATHNAME}.sh \
| awk '{print $4}' > ${JOB_OUTPUT_DIR}/${STEP}/${NOPATHNAME}.JOBID
